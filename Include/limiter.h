...
// Return Type  : void
//
void limiter(double n, const double f[1764], emxArray_real_T* df1,
	emxArray_real_T* df2)
{
	int k;
	int loop_ub;
	...
	k = df2->size[0] * df2->size[1];
	df2->size[0] = (int)n;
	df2->size[1] = (int)n;
	//emxEnsureCapacity_real_T(df2, k);
	loop_ub = (int)n * (int)n;
	for (k = 0; k < loop_ub; k++) {
		df2->data[k] = 0.0;
	}
	...
	for (k = 0; k < (int)n; k++) {
		df1->data[df1->size[0] * k] = 0.0;
		df1->data[k] = 0.0;
		df2->data[df2->size[0] * k] = 0.0;
		df2->data[k] = 0.0;
	}
	...
	for (loop_ub = 1; loop_ub - 1 < (int)((n - 1.0) + -1.0); loop_ub++) {
		for (k = 1; k - 1 < (int)((n - 1.0) + -1.0); k++) {
			...
			//  end
			df1->data[loop_ub + df1->size[0] * k] = df2x;
			df2->data[loop_ub + df2->size[0] * k] = df1x;
		}
	}
}

//
// File trailer for limiter.cpp
//
// [EOF]
//