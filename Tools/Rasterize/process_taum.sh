...
PREFIX="${3:-H}"
CFGFILE="${4:-${PROJECT_ROOT}/input/cfg/ts_dam_break.cfg}"
HYDROFILE="${5:-${PROJECT_ROOT}/input/hyg/ts_dam_break.hyg}"
DESC="${6:-Taum Sauk 60 minutes sim time}"
...
./rasterize --datadir="${PROJECT_ROOT}/output/${OUTPUT_NAME}" --nrows=1136 --ncols=624 --bgcolors=128 --fgcolors=128 --bgdata="${BGFILE}"
